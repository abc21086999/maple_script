...
## ✨ 主要特色
- **現代化控制中心**：深色主題的圖形介面。
- **硬體連線及軟體模擬**：預設需要開發板作為硬體，充當真實輸入來源；現在也支援軟體模擬輸入，不須準備硬體也可以使用。
- **路徑錄製與重播**：支援手動錄製練功循環路徑，並實現高精度的時間序列重播。
- **全自動化功能**：
  - **自動練功 (Auto Grind - 狀態機架構)**：
    - 採用狀態機設計，將練功行為拆分為獨立模組狀態：定點、走圖、亂逛、解輪與暫停。
    - 中斷機制：當偵測到不安全狀況（如偵測到紅點、出現符文、失去視窗焦點）時，狀態機會將當前狀態切換至中斷狀態；當環境恢復安全後，無縫恢復先前狀態。
