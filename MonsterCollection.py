...
class MonsterCollection(MapleScript):
    ...
    def collect_monster_collection(self):
        ...
        # 按下探險的那個Tab
        self.find_and_click_image("photos/adventure_tab.png")
        time.sleep(0.3)

        # 一直截圖進行辨識，如果畫面上還有還沒領取的怪物蒐藏
        while self.is_on_screen("photos/receive_monster_collection_reward.png", self.get_full_screen_screenshot()):
            # 按下領取按鈕
            self.find_and_click_image("photos/receive_monster_collection_reward.png")
            time.sleep(0.2)
